...
function createStoreStateHook(Context) {
  return function useStoreState(mapState, equalityFn) {
    ...
    */
    ...
    var selectedState = useSyncExternalStoreWithSelector(store.subscribe, store.getState, store.getServerState || store.getState, mapState, equalityFn);
...
var persist = function persist(model, config) {
  ...
  return (// if we are not running in a browser context this becomes a no-op
    typeof window === 'undefined' ? model : _objectSpread__default["default"](_objectSpread__default["default"]({}, model), {}, (_objectSpread2 = {}, _objectSpread2[persistSymbol] = config, _objectSpread2))
  );
};
...
function createSimpleProduce(disableImmer) {
  ...
  return function simpleProduce(path, state, fn, config) {
    ...
    if (path.length === 0) {
      ...
      if (_result) {
        return immer.isDraft(_result) ? easyPeasyImmer.finishDraft(_result) : _result;
      }
  ...
  var reducerForCustomReducers = function reducerForCustomReducers(state, action) {
    return _cR.reduce(function (acc, _ref) {
      ...
      return simpleProduce(parentPath, acc, function (draft) {
        draft[key] = reducer(immer.isDraft(draft[key]) ? immer.original(draft[key]) : draft[key], action);
        return draft;
      }, config);
    }, state);
  };
...
function createComputedPropertyBinder(parentPath, key, def, _r) {
  ...
  var areEqual = function areEqual(a, b) {
    ...
    var result = equal__default["default"](a, b);
  ...
  return function createComputedProperty(parentState, storeState) {
    Object.defineProperty(parentState, key, {
      ...
      get: function get() {
        ...
        if (runOnce && (storeState === prevStoreState || areInputsEqual(inputs, prevInputs) || // We don't want computed properties resolved every time an action
...
function createStore(model, options) {
  ...
  var bindReplaceState = function bindReplaceState(modelDef) {
    return _objectSpread__default["default"](_objectSpread__default["default"]({}, modelDef), {}, {
  ...
  var bindStoreInternals = function bindStoreInternals(state) {
    ...
    _r._i = _objectSpread__default["default"](_objectSpread__default["default"]({}, data), {}, {
  ...
  var composeEnhancers = compose || (devTools && typeof window !== 'undefined' && window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ ? window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__(_objectSpread__default["default"]({
    name: storeName
  }, devToolConfig)) : redux.compose);
  bindStoreInternals(initialState || {});
  var easyPeasyMiddleware = [createComputedPropertiesMiddleware(_r)].concat(middleware, [reduxThunk__default["default"], createListenerMiddleware(_r), createEffectsMiddleware(_r), persistMiddleware]);

  if (mockActions) {
    easyPeasyMiddleware.push(mockActionsMiddleware);
  }

  var store = redux.createStore(_r._i.reducer, _r._i._dS, composeEnhancers.apply(void 0, [redux.applyMiddleware.apply(void 0, easyPeasyMiddleware)].concat(enhancers)));
...
function createContextStore(model, config) {
  ...
  function Provider(_ref) {
    ...
    var store = useMemoOne(function () {
      return createStore(typeof model === 'function' ? model(runtimeModel) : model, _objectSpread__default["default"](_objectSpread__default["default"]({}, config), {}, {
...
initializeUseStoreState(withSelector.useSyncExternalStoreWithSelector);
...
exports.initializeUseStoreState = initializeUseStoreState;
